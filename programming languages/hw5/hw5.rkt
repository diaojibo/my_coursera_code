  ...
  (cond [(var? e) 
        ...
        [(fun? e)
         (letrec ([funname (fun-nameopt e)]
                  [funarg (fun-formal e)]
                  [newenv env] ;;put itself into environment
        ...
        [(call? e)
         (letrec(
           ...
           (if (closure? clos)
               (letrec (
                       [function (closure-fun clos)]
                       [funbody (fun-body function)]
                       [inval (fun-formal function)]
                       [fname (fun-nameopt function)]
                       [cloenv (closure-env clos)]
                       [newenv (if fname (cons (cons fname clos) cloenv) cloenv)]
                       )
                 (eval-under-env funbody (cons (cons inval arg) newenv)))
        ...
        [(fst? e)
         (letrec ([v (eval-under-env (fst-e e) env)])
           ...
           if (apair? v)
               (apair-e1 v)
               (error "Can only fall fst on a pair")))]
         [(snd? e)
         (letrec ([v (eval-under-env (snd-e e) env)])
           (if (apair? v)
               (apair-e2 v)
               (error "Can only fall snd on a pair")))]
         [(isaunit? e)
          (letrec(
...
(define mupl-map
  (fun "map" "f"
       (fun "map_list" "xs" (ifaunit (var "xs") (aunit) (apair (call (var "f") (fst (var "xs"))) (call (var "map_list") (snd (var "xs")))))))
  )

(define mupl-mapAddN 
  (mlet "map" mupl-map
        (fun "mapadd" "x" (call (var "map") (fun #f "y" (add (var "x") (var "y")))))))
