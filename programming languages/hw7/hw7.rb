...
class LineSegment < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
#puts seg.x1.to_s+" "+seg.y1.to_s+" "+seg.x2.to_s+" "+seg.y2.to_s
#puts x1.to_s+" "+y1.to_s+" "+x2.to_s+" "+y2.to_s
	if(real_close(seg.x1,seg.x2))
	  ...
	  if(real_close(small.y2,big.y1))
	    Point.new(small.x2,small.y2)
	  elsif(small.y2<big.y1 and !real_close(small.y2,big.y1))
		NoPoints.new()
	  elsif(small.y2>big.y2)
	    LineSegment.new(big.x1,big.y1,big.x2,big.y2)
	  else
	    LineSegment.new(big.x1,big.y1,small.x2,small.y2)
	  end
	else
	  if(@x1<seg.x1)
	    small = self
	  ...
	  if(real_close(small.x2,big.x1))
	    Point.new(small.x2,small.y2)
	  elsif(small.x2<big.x1 and !real_close(small.x2,big.x1))
	    NoPoints.new()
	  elsif(small.x2>big.x2)
	    LineSegment.new(big.x1,big.y1,big.x2,big.y2)
	  else
	    LineSegment.new(big.x1,big.y1,small.x2,small.y2)
	  end
...
class Let < GeometryExpression
  ...
  def eval_prog env # remember: do not change this method
    a1 = @e1.eval_prog env
	a1 = a1.preprocess_prog
	newenv = env.insert(0,Array[@s,a1])
	e2 = @e2.eval_prog newenv
	e2.preprocess_prog
  end
  def preprocess_prog
    self # no pre-processing to do here
